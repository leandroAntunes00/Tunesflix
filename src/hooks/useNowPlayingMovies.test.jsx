...
function TestComp({ initialPage = 1 }) {
  const {
    ...
    prevPage,
    reset,
  } = useNowPlayingMovies(initialPage);
...
describe('useNowPlayingMovies hook', () => {
  beforeEach(() => {
    vi.resetAllMocks();
  });
  ...
  describe('Estados computados', () => {
    test('hasResults reflete corretamente o estado', async () => {
      ...
      expect(await screen.findByTestId('result-40')).toHaveTextContent('Now Playing Movie');
    ...
    test('canGoNext funciona corretamente', async () => {
      ...
      await screen.findByTestId('result-40');
  ...
  describe('Busca de filmes', () => {
    test('carrega e popula resultados', async () => {
      ...
      expect(screen.getByTestId('loading').textContent).toBe('false');
      expect(screen.getByTestId('page').textContent).toBe('1');
      expect(screen.getByTestId('totalPages').textContent).toBe('207');
      expect(screen.getByTestId('totalResults').textContent).toBe('4128');
    });
  ...
  describe('Tratamento de erros', () => {
    test('lida com erro na busca', async () => {
      ...
      fetchNowPlayingMovies.mockRejectedValueOnce(new Error('Network error'));
    ...
    test('lida com resposta inválida', async () => {
      const { fetchNowPlayingMovies } = await import('../services/tmdb');
      fetchNowPlayingMovies.mockResolvedValueOnce(null);
  ...
  describe('Condições de corrida', () => {
    test('ignora respostas antigas', async () => {
      ...
      fetchNowPlayingMovies.mockReturnValueOnce(firstPromise).mockResolvedValueOnce({
        results: [{ id: 50, title: 'Second Movie' }],
        page: 1,
        total_pages: 1,
        total_results: 1,
      });
      ...
      firstResolve({
        results: [{ id: 40, title: 'First Movie' }],
        page: 1,
        total_pages: 1,
        total_results: 1,
      });
      ...
      expect(await screen.findByTestId('result-50')).toHaveTextContent('Second Movie');
    });
  });
