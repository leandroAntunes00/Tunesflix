...
describe('useTopRatedMovies hook', () => {
  beforeEach(() => {
    vi.resetAllMocks();
  });
  ...
  describe('Estados computados', () => {
    test('hasResults reflete corretamente o estado', async () => {
      ...
      expect(await screen.findByTestId('result-30')).toHaveTextContent('Top Rated Movie');
    ...
    test('canGoNext funciona corretamente', async () => {
      ...
      await screen.findByTestId('result-30');
  ...
  describe('Busca de filmes', () => {
    test('carrega e popula resultados', async () => {
      ...
      expect(screen.getByTestId('loading').textContent).toBe('false');
      expect(screen.getByTestId('page').textContent).toBe('1');
      expect(screen.getByTestId('totalPages').textContent).toBe('516');
      expect(screen.getByTestId('totalResults').textContent).toBe('10308');
    });
  ...
  describe('Tratamento de erros', () => {
    test('lida com erro na busca', async () => {
      const { fetchTopRatedMovies } = await import('../services/tmdb');
      fetchTopRatedMovies.mockRejectedValueOnce(new Error('Network error'));
    ...
    test('lida com resposta inválida', async () => {
      const { fetchTopRatedMovies } = await import('../services/tmdb');
      fetchTopRatedMovies.mockResolvedValueOnce(null);
  ...
  describe('Condições de corrida', () => {
    test('ignora respostas antigas', async () => {
      ...
      fetchTopRatedMovies.mockReturnValueOnce(firstPromise).mockResolvedValueOnce({
        results: [{ id: 35, title: 'Second Movie' }],
        page: 1,
        total_pages: 1,
        total_results: 1,
      });
      ...
      firstResolve({
        results: [{ id: 30, title: 'First Movie' }],
        page: 1,
        total_pages: 1,
        total_results: 1,
      });
      ...
      expect(await screen.findByTestId('result-35')).toHaveTextContent('Second Movie');
    });
  });
